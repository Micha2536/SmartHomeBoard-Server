...
def shell(title, subtitle, body, version, active="dashboard"):
    items = [
        ...
        ("audit", "/setup/audit", "Schaltprotokoll"),
...
def command_audit_page(version, events, nodes=(), filters=None):
    filters = filters or {}
    selected_node = str(filters.get("node_id", ""))
    selected_module = str(filters.get("integration_module", ""))
    selected_kind = str(filters.get("event_kind", ""))
    selected_source = str(filters.get("source", ""))
    node_options = ['<option value="">Alle Geräte</option>']
    for node in sorted(nodes, key=lambda item: display_text(item.get("name", "")).casefold()):
        value = str(node.get("id", ""))
        node_options.append(
            f'<option value="{escape(value)}" {"selected" if value == selected_node else ""}>'
            f'{display_html(node.get("name") or f"Gerät {value}")}</option>'
        )
    modules = sorted({str(node.get("integration_module", "")) for node in nodes if node.get("integration_module")})
    module_options = ['<option value="">Alle Integrationen</option>'] + [
        f'<option value="{escape(value)}" {"selected" if value == selected_module else ""}>{escape(value)}</option>'
        for value in modules
    ]
    kind_options = "".join(
        f'<option value="{value}" {"selected" if value == selected_kind else ""}>{label}</option>'
        for value, label in (("", "Befehle und Rückmeldungen"), ("command", "Nur Schaltbefehle"), ("state_change", "Nur Zustandsänderungen"))
    )
    sources = sorted({"automation", "ios_app", "web_portal", "server_api", "device_or_external", selected_source} - {""})
    source_options = ['<option value="">Alle Auslöser</option>'] + [
        f'<option value="{escape(value)}" {"selected" if value == selected_source else ""}>{escape(_audit_source_title(value))}</option>'
        for value in sources
    ]
    rows = []
    for item in events:
        old = "–" if item.get("previous_value") is None else display_html(item.get("previous_value"))
        target = "–" if item.get("requested_value") is None else display_html(item.get("requested_value"))
        observed = "–" if item.get("observed_value") is None else display_html(item.get("observed_value"))
        kind = "Befehl" if item.get("event_kind") == "command" else "Rückmeldung"
        status = str(item.get("status", ""))
        status_class = "bad" if status == "failed" else "ok" if status in ("confirmed", "observed") else ""
        detail = display_html(item.get("source_detail") or item.get("client_id") or "–")
        error = f'<small style="color:var(--bad)">{display_html(item.get("error"))}</small>' if item.get("error") else ""
        rows.append(f'''<tr><td><time class="audit-time" data-ts="{escape(str(item.get('created_at','')))}">{escape(str(item.get('created_at','')))}</time></td>
<td><b>{display_html(item.get('node_name') or f"Gerät {item.get('node_id','')}")}</b><small>{display_html(item.get('attribute_name') or f"Attribut {item.get('attribute_id','')}")} · {display_html(item.get('integration_module') or '')}</small></td>
<td><span class="badge">{kind}</span></td><td><b>{escape(_audit_source_title(str(item.get('source',''))))}</b><small>{detail}</small></td>
<td><code>{old}</code> → <code>{target}</code><small>Gemeldet: <code>{observed}</code></small></td><td><span class="badge {status_class}">{escape(status)}</span>{error}</td></tr>''')
    rows_html = "".join(rows) or '<tr><td colspan="6" class="muted">Für diesen Filter liegen noch keine Schaltvorgänge vor.</td></tr>'
    body = f'''<section class="panel"><div class="notice"><b>Diagnosehinweis</b><br>„Extern/Gerät“ bedeutet: Der Server hat eine Zustandsänderung empfangen, aber in den 15 Sekunden davor keinen passenden SHB-Schaltbefehl registriert. Prüfe dann Shelly-Zeitpläne, Scripts, Eingänge, Cloud und weitere Steuerzentralen.</div>
<form method="get" action="/setup/audit"><div class="stats" style="grid-template-columns:repeat(4,minmax(0,1fr))"><label>Gerät<select name="node_id">{''.join(node_options)}</select></label><label>Integration<select name="integration_module">{''.join(module_options)}</select></label><label>Eintrag<select name="event_kind">{kind_options}</select></label><label>Auslöser<select name="source">{''.join(source_options)}</select></label></div><div class="actions"><button>Filtern / aktualisieren</button><a class="button secondary" href="/setup/audit">Filter löschen</a></div></form></section>
<section class="panel" style="overflow:auto"><table style="width:100%;border-collapse:collapse;min-width:980px"><thead><tr><th>Zeitstempel</th><th>Gerät / Attribut</th><th>Art</th><th>Auslöser</th><th>Werte</th><th>Status</th></tr></thead><tbody>{rows_html}</tbody></table></section>
<style>th,td{{text-align:left;vertical-align:top;padding:10px;border-bottom:1px solid var(--line)}}td small{{display:block;margin-top:4px}}time{{white-space:nowrap;font-variant-numeric:tabular-nums}}</style>
<script>document.querySelectorAll('.audit-time').forEach(el=>{{const date=new Date(el.dataset.ts);if(!Number.isNaN(date.valueOf()))el.textContent=date.toLocaleString('de-DE',{{year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',fractionalSecondDigits:3}})}});</script>'''
    return shell("Schaltprotokoll", "Persistente Befehls- und Zustandsdiagnose mit Auslöserzuordnung", body, version, "audit")


def _audit_source_title(source):
    return {
        "automation": "Automation",
        "ios_app": "iOS-/Mac-App",
        "web_portal": "Webportal",
        "server_api": "API-Client",
        "device_or_external": "Extern / Gerät",
    }.get(source, source or "Unbekannt")
...
def integrations_page(version, manifests, integrations, selected_module="", selected_id="", authenticated=False, token_required=False, message="", error="", homee_protocol=None, protocol_filter="", nodes=()):
    ...
    if manifest:
        ...
        if selected:
            ...
            if module_id == "enocean":
                editor += f'''<section style="margin-top:22px;border-top:1px solid var(--line);padding-top:18px"><h2>EnOcean-Geräte anlernen</h2><p class="muted">Der F6-Fallback oben ist nur eine Empfangseinstellung für unbekannte Taster. Geräte werden mit vollständiger Profilauswahl und passender Anleitung im EnOcean-Gerätebereich angelernt.</p><a class="button" href="/setup/enocean?integration={escape(selected["id"])}">EnOcean verwalten und anlernen</a></section>'''
            ...
            integration_nodes = [node for node in nodes if str(node.get("integration_id", "")) == str(selected["id"])]
            editor += _integration_devices_html(selected["id"], integration_nodes, token_required, authenticated)
            if module_id == "homee":
...
def _integration_devices_html(integration_id, nodes, token_required, authenticated):
    if not nodes:
        rows = '<p class="muted">Diese Integration hat noch keine Geräte geliefert.</p>'
    else:
        rows = "".join(
            f'''<div class="row"><span><b>{display_html(node.get("name") or f'Gerät {node.get("id", "")}' )}</b><small>{len(node.get("attributes") or [])} Werte · ID {int(node.get("id", 0))}</small></span><form method="post" action="/setup/integrations/device-visibility">{token_field(token_required,authenticated)}<input type="hidden" name="integration_id" value="{escape(str(integration_id))}"><input type="hidden" name="node_id" value="{int(node.get("id", 0))}"><input type="hidden" name="enabled" value="{'0' if node.get('dashboard_enabled', True) else '1'}"><button type="submit" class="{'secondary' if node.get('dashboard_enabled', True) else 'danger'}">{'Aktiv' if node.get('dashboard_enabled', True) else 'Nicht aktiv'}</button></form></div>'''
            for node in sorted(nodes, key=lambda item: display_text(item.get("name", "")).casefold())
        )
    return f'''<section style="margin-top:22px;border-top:1px solid var(--line);padding-top:18px"><h2>Geräte dieser Integration · {len(nodes)}</h2><p class="muted">Nicht aktive Geräte bleiben gespeichert und werden weiter aktualisiert, erscheinen aber nicht im App-Dashboard. Neue Geräte sind standardmäßig aktiv.</p><div class="list">{rows}</div></section>'''
...
def _action_field_html(field):
    ...
    if kind == "select":
        options = "".join(
            f'<option value="{escape(str(option.get("value", "")))}">{display_html(option.get("label", option.get("value", "")))}</option>'
            for option in field.get("options", [])
        )
        return f'<label>{display_html(title)}<select name="payload__{escape(key)}" {required}>{options}</select>{help_html}</label>'
...
function conditionKinds(trigger){{return trigger?[['attribute','Gerätewert'],['attributeChanged','Gerätewert ändert sich'],['attributeChangedBy','Gerätewert ändert sich um'],['timeDaily','Täglich um'],['timeOnce','Einmalig am']]:[['attribute','Gerätewert'],['timeAfter','Uhrzeit nach'],['timeBefore','Uhrzeit vor']]}}
