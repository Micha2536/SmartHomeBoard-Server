...
def automations_page(version, status, authenticated=False, token_required=False, message="", error=""):
    cards = "".join(f'<div class="row"><span><b>{escape(item["name"])}</b><small>{item.get("trigger_count",0)} Auslöser · {item.get("action_count",0)} Aktionen</small></span><span class="badge {"ok" if item.get("enabled") else ""}">{"Aktiv" if item.get("enabled") else "Aus"}</span></div>' for item in status.get("automations", [])) or '<p class="muted">Noch keine Automationen von der App übertragen.</p>'
    options = "".join(f'<option value="{escape(item["id"])}">{escape(item["name"])}</option>' for item in status.get("automations", []))
    ...
    body = f'''{notice(message,error)}<div class="split"><section class="panel"><h2>Serverregeln · {status.get("count",0)}</h2><div class="list">{cards}</div></section><section><div class="panel"><h2>Automation testen</h2><p class="muted">Der Test führt die Aktionen wirklich aus.</p><form method="post" action="/setup/automations/test">{token_field(token_required,authenticated)}<label>Automation<select name="rule_id" required>{options}</select></label><button {"" if options else "disabled"}>Jetzt testen</button></form></div><div class="panel"><h2>Letzte 12 Ereignisse</h2><div class="list">{events}</div></div></section></div>'''
    return shell("Automationen", "Laufen auch dann weiter, wenn iPad und App nicht verbunden sind.", body, version, "automations")
